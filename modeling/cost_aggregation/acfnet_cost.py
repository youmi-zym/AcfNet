...
from ..basic_layers import conv3d_bn, conv3d_bn_relu
from .utils.hourglass import Hourglass
...
class AcfCost(nn.Module):
    ...
    Outputs:
        cost_volume, (tuple of Tensor): cost volume in (BatchSize, MaxDisparity, Height, Width) layout
    """

    def __init__(self, max_disp, in_planes=64, batchNorm=True):
        super(AcfCost, self).__init__()
        self.max_disp = max_disp
        self.in_planes = in_planes
        self.batchNorm = batchNorm

        self.dres0 = nn.Sequential(
            conv3d_bn_relu(self.batchNorm, self.in_planes, 32, 3, 1, 1),
            conv3d_bn_relu(self.batchNorm, 32, 32, 3, 1, 1),
        )
        self.dres1 = nn.Sequential(
            conv3d_bn_relu(self.batchNorm, 32, 32, 3, 1, 1),
            conv3d_bn(self.batchNorm, 32, 32, 3, 1, 1)
        )
        self.dres2 = Hourglass(in_planes=32, batchNorm=self.batchNorm)
        self.dres3 = Hourglass(in_planes=32, batchNorm=self.batchNorm)
        self.dres4 = Hourglass(in_planes=32, batchNorm=self.batchNorm)
